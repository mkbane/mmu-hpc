...
__global__ void vecdiv(float *B, float *C, float *A, int N)
{
  // form A = B / C
...
void serial_vecdiv(float *B, float *C, float *A, int N)
{
  // form A = B / C
  int idx;
...
int main(int argc, char *argv[]) {
  ...
  float *A, *B, *C;
   ...
   vecdiv <<<blocks, threadsPerBlock>>> (d_B, d_C, d_A, N);
